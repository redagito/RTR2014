...
bool CSceneLoader::load(const std::string& file, IScene& scene)
{
	...
	{
		LOG_ERROR("Failed to load scene file %s with error %s.", file.c_str(), reader.getFormattedErrorMessages().c_str());
		return false;
	}
	ifs.close();
	...
	if (root.empty())
	{
		LOG_ERROR("Missing or empty 'scene_objects' node in json file %s. No scene objects are loaded.", file.c_str());
		return false;
	}
	...
	for (unsigned int i = 0; i < sceneObjects.size(); ++i)
	{
		...
		if (position.size() != 3)
		{
			LOG_ERROR("In json file %s, the 'position' parameter does not have size 3.", file.c_str());
			return false;
		}
		if (rotation.size() != 3)
		{
			LOG_ERROR("In json file %s, the 'rotation' parameter does not have size 3.", file.c_str());
			return false;
		}
		if (scale.size() != 3)
		{
		...
		{
			LOG_ERROR("In json file %s, the 'position' parameter contains a non-numeric value.", file.c_str());
			return false;
		}
		if (!rotation[0].isNumeric() || !rotation[1].isNumeric() || !rotation[2].isNumeric())
		{
			LOG_ERROR("In json file %s, the 'rotation' parameter contains a non-numeric value.", file.c_str());
			return false;
		}
		if (!scale[0].isNumeric() || !scale[1].isNumeric() || !scale[2].isNumeric())
		{
			LOG_ERROR("In json file %s, the 'scale' parameter contains a non-numeric value.", file.c_str());
			return false;
		}
		...
		if (meshId == -1)
		{
			LOG_ERROR("In json file %s, failed to load mesh file %s.", file.c_str(), mesh.asCString());
			return false;
		}
		...
		if (materialId == -1)
		{
			LOG_ERROR("In json file %s, failed to load material file %s.", file.c_str(), material.asCString());
			return false;
		}
