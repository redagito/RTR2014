...
ResourceId CResourceManager::loadMesh(const std::string& file)
{
	...
	ResourceId meshId = createMesh(shapes.at(0).mesh.positions, shapes.at(0).mesh.indices, shapes.at(0).mesh.normals, shapes.at(0).mesh.texcoords, EPrimitiveType::Triangle);
	if (meshId == -1)
	{
...
ResourceId CResourceManager::loadMaterial(const std::string& file)
{
	...
	if (ini.hasKey("diffuse", "file"))
	{
		...
		if (diffuseId == -1)
		{
			LOG_ERROR("Failed to load diffuse texture specified in material file %s.", file.c_str());
			return -1;
		}
	}
	...
	if (ini.hasKey("specular", "file"))
	{
		...
		if (specularId == -1)
		{
			LOG_ERROR("Failed to load specular texture specified in material file %s.", file.c_str());
			return -1;
		}
	}
	...
	if (ini.hasKey("shader", "file"))
	{
		...
		if (customShaderId == -1)
		{
			LOG_ERROR("Failed to load custom shader file specified in material file %s.", file.c_str());
			return -1;
		}
	}

	ResourceId materialId = createMaterial(diffuseId, alphaId, normalId, specularId, glowId, customShaderId);
	if (materialId == -1)
	{
...
ResourceId CResourceManager::loadShader(const std::string& file)
{
	...
	{
		LOG_ERROR("The shader program file %s is missing a vertex and/or fragment shader source file specifier.", file.c_str());
		return -1;
	}
	...
	if (vertexId == -1)
	{
		LOG_ERROR("The vertex shader source file %s, specified in the shader program file %s could not be loaded.", ini.getValue("vertex", "file", "error").c_str(), file.c_str());
		return -1;
	}
	...
	if (fragmentId == -1)
	{
		LOG_ERROR("The fragment shader source file %s, specified in the shader program file %s could not be loaded.", ini.getValue("fragment", "file", "error").c_str(), file.c_str());
		return -1;
	}
	...
	if (ini.hasKey("tessellation_control", "file"))
	{
		...
		if (tessCtrlId == -1)
		{
			LOG_ERROR("The tessellation control shader source file %s, specified in the shader program file %s could not be loaded.", ini.getValue("tessellation_control", "file", "error").c_str(), file.c_str());
			return -1;
		}
	}
	...
	if (ini.hasKey("tessellation_evaluation", "file"))
	{
		...
		if (tessEvalId == -1)
		{
			LOG_ERROR("The tessellation evaluation shader source file %s, specified in the shader program file %s could not be loaded.", ini.getValue("tessellation_evaluation", "file", "error").c_str(), file.c_str());
			return -1;
		}
	}
	...
	if (ini.hasKey("geometry", "file"))
	{
		...
		if (geometryId == -1)
		{
			LOG_ERROR("The geometry shader source file %s, specified in the shader program file %s could not be loaded.", ini.getValue("geometry", "file", "error").c_str(), file.c_str());
			return -1;
		}
	}
